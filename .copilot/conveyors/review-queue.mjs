...
export const meta = {
	name: "review-queue",
	description: "Review supplied PR diffs, verify low-risk candidates, and render a triage queue.",
	limits: {
...
const conveyorArgs = context.args;
const opts =
	conveyorArgs && typeof conveyorArgs === "object" && !Array.isArray(conveyorArgs)
		? conveyorArgs
		: {};
const input = opts.prs ?? conveyorArgs;
...
phase("Review");
const reviewed = await pipeline(prs, async (pr) => {
...
phase("Verify");
const rows = await pipeline(reviewed, async (row, _original, index) => {
	if (row === null) {
		return {
			pr: prs[index],
