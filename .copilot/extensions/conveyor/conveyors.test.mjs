/** @module conveyors.test — saved conveyors: parse, meta, and control-flow smoke runs. */
...
const CONVEYORS = resolve(dirname(fileURLToPath(import.meta.url)), "../../conveyors");
const EXAMPLES = resolve(dirname(fileURLToPath(import.meta.url)), "../../skills/conveyor/examples");
const SECURITY_HOST = "../../conveyors/security-review.host.mjs";
const REVIEW_QUEUE_HOST = "../../conveyors/review-queue.host.mjs";
...
/**
 * Build a permissive harness API that lets any conveyor run its control flow to
...
function smokeApi(args, behavior = {}) {
	...
	const worktree = async (/** @type {string} */ name, /** @type {any} */ a, /** @type {any} */ b) => {
		...
		return cb ? await cb("/tmp/conveyor-smoke-wt") : { path: "/tmp/conveyor-smoke-wt", cleanup: async () => {} };
	};
	worktree.create = async () => ({ path: "/tmp/conveyor-smoke-wt", cleanup: async () => {} });
	const api = /** @type {any} */ ({
		context: {
			args,
...
async function smoke(dir, name, args, behavior = {}) {
	...
	assert.ok(existsSync(path), `missing conveyor ${name}.mjs`);
...
test("audit.mjs: verifies findings and synthesizes a report", async () => {
	const { result, calls, meta } = await smoke(CONVEYORS, "audit", ["a.js", "b.js"]);
	...
	await assert.rejects(smoke(CONVEYORS, "audit", []), /provide files/);
});

test("audit.mjs: file/verifier failures stay explicit and verification can read original evidence", async () => {
	const run = await smoke(CONVEYORS, "audit", ["a.js", "b.js"], {
...
test("triage.mjs: emits structured priority/confidence/action rows", async () => {
	const { result, calls, meta } = await smoke(CONVEYORS, "triage", ["t1", "t2"]);
	...
	await assert.rejects(smoke(CONVEYORS, "triage", []), /non-empty array/);
});

test("triage.mjs: one malformed classification does not abort healthy siblings", async () => {
	const run = await smoke(CONVEYORS, "triage", ["bad", "good"], {
...
test("deep-research.mjs: plans angles, verifies, and synthesizes only verified research", async () => {
	const { result, calls, meta } = await smoke(CONVEYORS, "deep-research", "What is X?");
	...
	assert.ok(calls.verify >= 1);
	await assert.rejects(smoke(CONVEYORS, "deep-research", null), /non-empty question/);
});

test("deep-research.mjs: planner is no-tools, source checks can browse, and rejected research is never synthesized", async () => {
	const run = await smoke(CONVEYORS, "deep-research", { question: "What is X?", angles: 2 }, {
...
test("deep-research.mjs: dry-run estimates requested research and verification arity", async () => {
	const run = await smoke(CONVEYORS, "deep-research", { question: "What is X?", angles: 5 }, { dryRun: true });
...
test("deep-research: every worker and verifier retains context without duplicating it in the rubric", async () => {
	...
	const cases = [
		[CONVEYORS, { question, angles: angles.length }],
		[EXAMPLES, question],
	];
...
test("review-queue.mjs: diff-only and deep-checkout PRs render a triage table", async () => {
	...
	const { result, calls, meta } = await smoke(CONVEYORS, "review-queue", prs);
	...
	const empty = await smoke(CONVEYORS, "review-queue", []);
...
test("review-queue.mjs: diff agents are no-tools while checkout agents are read-only", async () => {
	...
	const { calls } = await smoke(CONVEYORS, "review-queue", prs);
...
test("review-queue.mjs: CODEOWNERS uses documented per-file GitHub semantics", async () => {
	...
	const { result } = await smoke(CONVEYORS, "review-queue", { prs, auto_deep: false });
...
test("review-queue.mjs: chunk caps are explicit and block approval", async () => {
	...
	const capped = await smoke(CONVEYORS, "review-queue", { prs: [pr], auto_deep: false, diff_chunk_chars: 4000, max_chunks: 1 });
	...
	const complete = await smoke(CONVEYORS, "review-queue", { prs: [pr], auto_deep: false, diff_chunk_chars: 4000, max_chunks: 20 });
...
test("review-queue.mjs: a PR larger than the old 40-chunk limit is fully reviewed", async () => {
	...
	const run = await smoke(CONVEYORS, "review-queue", { prs: [pr], auto_deep: false });
...
test("review-queue.mjs: an unsplittable diff line is explicit and blocks approval", async () => {
	...
	const run = await smoke(CONVEYORS, "review-queue", { prs: [pr], auto_deep: false, diff_chunk_chars: 4000 });
...
test("review-queue.mjs: review or verifier failures fail closed without aborting siblings", async () => {
	...
	const malformed = await smoke(CONVEYORS, "review-queue", { prs, auto_deep: false }, {
		structured: (_prompt, schema, _options, index) => {
	...
	const rejected = await smoke(CONVEYORS, "review-queue", { prs: [prs[0]], auto_deep: false }, {
...
test("review-queue.mjs: checkout fallback is degraded and cannot approve", async () => {
	...
	const { result } = await smoke(CONVEYORS, "review-queue", { prs: [pr], deep: true }, { worktreeError: "checkout failed" });
...
test("review-queue.mjs: missing or symlinked checkout files cannot approve", async () => {
	...
	const run = await smoke(CONVEYORS, "review-queue", { prs: [pr], deep: true }, {
...
test("review-queue fetcher reads CODEOWNERS from the PR base branch", () => {
	const fetcher = readFileSync(resolve(CONVEYORS, "../skills/review-queue/scripts/review-queue-fetch.sh"), "utf8");
...
test("security-review.mjs: scans, investigates, verifies, and renders a report", async () => {
	const { result, calls, meta } = await smoke(CONVEYORS, "security-review", { root: "src/" });
...
test("security-review.mjs: uncached host evidence and failures stay explicit", async () => {
	...
	const run = await smoke(CONVEYORS, "security-review", { root: "src" }, {
		host,
...
test("security-review.mjs: dry-run follows discovered batch and verification fan-out", async () => {
	const { calls } = await smoke(CONVEYORS, "security-review", { root: "src" }, { dryRun: true });
...
test("repo conveyor/example harnesses use .mjs names", () => {
	for (const name of ["audit", "triage", "deep-research", "review-queue", "security-review"]) {
		assert.ok(existsSync(join(CONVEYORS, `${name}.mjs`)), `${name}.mjs exists`);
	}
	for (const name of EXAMPLE_NAMES) {
...
test("every checked-in conveyor and example declares a usable meta block", () => {
	for (const dir of [CONVEYORS, EXAMPLES]) {
