 ...
 * @module extension
 *
 * conveyor extension — the only SDK-coupled module. It joins the Copilot session, adapts it into a
 * {@link import("./tools.mjs").ToolCtx} (`log` / `send` / `getWorkspaceCwd`), and registers the
 * conveyor tools built by `tools.mjs`. The conveyor engine (`runtime`/`agent`/`sandbox`/...) is pure Node
...
const ctx = {
	...
	send(prompt) {
		...
		const failed = (/** @type {unknown} */ e) => ctx.log(`conveyor: completion notice was not delivered: ${e instanceof Error ? e.message : e}`, false, "warning");
		try {
