 ...
 *
 * A zero-cost stand-in for the real `copilot` binary, used by the conveyor test suite
 * (`CONVEYOR_COPILOT_BIN` points at this file). It mimics `copilot -p <prompt> --output-format json`:
 ...
 * real AI credits:
 *
 * - `CONVEYOR_FAKE_MODE`   — `ok` (default) | `fail` | `hang` | `treehang` | `malformed` | `nojson` | `sessionerror`
 * - `CONVEYOR_FAKE_NANO_AIU` — session-wide nanoAIU to report (default `500000000` = 0.5 AIC)
 * - `CONVEYOR_FAKE_OUTPUT_TOKENS` — output tokens to report (default `42`)
 * - `CONVEYOR_FAKE_CONTENT` — assistant content (default `ECHO: <prompt>`)
 * - `CONVEYOR_FAKE_STDERR` — stderr text for `fail` mode
 * - `CONVEYOR_FAKE_SESSION` — force a child session id (default: the `--session-id` argument, else random)
 * - `CONVEYOR_FAKE_DELAY_MS` — delay a successful response (for concurrency tests)
 * - `CONVEYOR_FAKE_PID_FILE` — write this fake process's PID to the given path
...
const model = argValue(argv, "--model") ?? process.env.CONVEYOR_FAKE_MODEL ?? "fake-model";
const mode = process.env.CONVEYOR_FAKE_MODE ?? "ok";
const nanoAiu = Number(process.env.CONVEYOR_FAKE_NANO_AIU ?? 500_000_000);
const outputTokens = Number(process.env.CONVEYOR_FAKE_OUTPUT_TOKENS ?? 42);
const content = process.env.CONVEYOR_FAKE_CONTENT ?? `ECHO: ${prompt}`;
const resumedSessionId = argValue(argv, "--resume");
const sessionId = resumedSessionId ?? process.env.CONVEYOR_FAKE_SESSION ?? argValue(argv, "--session-id") ?? `fake-${randomBytes(6).toString("hex")}`;
const copilotHome = process.env.COPILOT_HOME || join(homedir(), ".copilot");
const delayMs = Math.max(0, Number(process.env.CONVEYOR_FAKE_DELAY_MS ?? 0));
if (process.env.CONVEYOR_FAKE_PID_FILE) writeFileSync(process.env.CONVEYOR_FAKE_PID_FILE, String(process.pid), "utf8");
...
function writeShutdownLog() {
	...
	if (process.env.CONVEYOR_FAKE_NANO_AIU_2) {
		appendFileSync(path, JSON.stringify(mkShutdown(Number(process.env.CONVEYOR_FAKE_NANO_AIU_2), usage)) + "\n", "utf8");
	}
}
...
switch (mode) {
	case "fail":
		process.stderr.write(process.env.CONVEYOR_FAKE_STDERR ?? "fake-copilot: simulated failure\n");
		process.exit(1);
		break;
	case "hang":
	...
	case "treehang": {
		...
		if (process.env.CONVEYOR_FAKE_PID_FILE && leaf.pid) writeFileSync(process.env.CONVEYOR_FAKE_PID_FILE, String(leaf.pid), "utf8");
		setInterval(() => {}, 1 << 30);
		break;
	}
	case "malformed":
