...
export class Lease {
	...
	assertOwned() {
		...
		if (owner?.token !== this.token || owner?.generation !== this.generation) {
			throw new LostLeaseError(`conveyor run '${this.persistence.runId}' ownership changed; refusing a stale write`);
		}
	}
		...
		while (true) {
			...
			try {
				...
				if (processAlive(Number(current?.pid))) {
					throw new LockedError(`conveyor run '${this.runId}' is owned by live process ${current.pid}`);
				}
