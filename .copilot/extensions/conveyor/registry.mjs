/** @module registry — project/user conveyor discovery with nearest-scope precedence. */
...
/** Closest project conveyor directories first. @param {string} cwd */
export function projectConveyorDirs(cwd) {
	const repo = findRepositoryRoot(cwd);
	if (!repo) return [];
	const dirs = [];
	let dir = resolve(cwd);
	while (true) {
		dirs.push(join(dir, ".copilot", "conveyors"));
		if (dir === repo) break;
		dir = dirname(dir);
	}
	return dirs;
...
/**
 * Resolve a conveyor name. Project definitions win by nearest directory, then user scope.
 * @param {string} name
 * @param {{ cwd: string, userDir: string }} opts
 * @returns {{ path: string, scope: "project"|"user", root: string }|null}
 */
export function resolveConveyorDefinition(name, { cwd, userDir }) {
	for (const dir of projectConveyorDirs(cwd)) {
