...
export function listConveyorRuns() {
	...
	for (const { name, d, mtime } of entries.slice(0, RUN_LIST_LIMIT)) {
		...
		rows.push({
			runId: name,
			status: rec.status || "?",
			conveyor: conveyorName(meta, rec),
...
export function loadConveyorResult(requestedRunId) {
	...
	if (typeof artifact.result !== "string") {
		throw new ConveyorResultError(`conveyor result for '${runId}' has a non-string result.`);
	}
	return {
		runId,
...
export function inspectConveyorRun(input) {
	...
	const conveyor = inspectString(conveyorName(meta, rec), INSPECT_LABEL_CHARS);
...
export function inspectConveyorAgent(input) {
	...
	if (!["summary", "prompt", "result", "events", "usage"].includes(section)) throw new ConveyorResultError("section must be summary, prompt, result, events, or usage.");
	const offset = resultInteger(input?.offset ?? 0, "offset");
	const limit = resultInteger(input?.limit ?? MAX_RESULT_CHUNK_CHARS, "limit");
	...
	if (sub === "result" || sub === "artifacts") {
		...
		if (sub === "result") {
			try {
				...
				return void log(loaded.result !== null ? loaded.result || "(no result text)" : "conveyor: no result yet (run may still be in progress).");
			} catch (e) {
