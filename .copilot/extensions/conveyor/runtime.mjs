...
export class Runtime {
	...
	constructor(opts = {}) {
		...
		// The launch-time budget is immutable from conveyor source.
		const rt = this;
		this.budget = {
			get total() {
	...
	async #worktreeCreate(name, opts = {}) {
		if (this.restricted) throw new Error("conveyor: worktree() is forbidden in restricted mode");
		if (this.dryRun) {
	 ...
	 * per-run temp base (`$TMPDIR/conveyor-wt-*`), outside the repo, so they leave no repo footprint and
	...
	async #managerFor(repo, cloneDir) {
		this.#wtBase ??= mkdtempSync(join(tmpdir(), "conveyor-wt-"));
		/** @type {string} */
		let root;
		if (repo == null) {
	 ...
	 * and prune. The per-run temp base (`$TMPDIR/conveyor-wt-*`) is deleted only when nothing dirty was
	 ...
	 * `followUp` on a replayed result resumes the recorded session id). `CONVEYOR_KEEP_SESSIONS=1` keeps
	...
	async #concurrentMap(n, work, opts) {
		...
		if (!["keep", "drop", "raise"].includes(failureMode)) throw new Error(`conveyor: onFailure must be keep, drop, or raise (got ${failureMode})`);
	...
	#buildSpec(o, internal = {}) {
		...
		if (removed.length) {
			throw new Error(`conveyor: removed agent option(s): ${removed.join(", ")}. Use profile, tools, permissions, and mcp instead.`);
		}
		if (o.mcp != null && !["inherit", "off"].includes(o.mcp)) {
			throw new Error("conveyor: agent mcp must be 'inherit' or 'off'; arbitrary MCP configuration belongs at conveyor launch");
		}
		if (this.restricted) {
			if (!["none", "read-only"].includes(o.profile ?? "read-only")) {
				throw new Error(`conveyor: restricted mode forbids agent profile '${o.profile}'`);
			}
			if (o.isolation) throw new Error("conveyor: restricted mode forbids tool-escalation options: isolation");
		}
		if (!this.dryRun && this.#parentPermissionMode === "off" && ["read-only", "research"].includes(o.profile)) {
			throw new Error(`conveyor: profile '${o.profile}' requires parent allow-all 'on' or 'auto'; fine-grained parent rules are not exposed authoritatively. Change the parent permission mode or use profile 'none'.`);
		}
		...
		if (spec.cwd && !internal.trustedCwd && !this.#cwdAllowed(spec.cwd)) {
			throw new Error(`conveyor: agent cwd is outside the parent-approved directories: ${spec.cwd}`);
		}
		return spec;
	}
	...
	#capItems(count, kind) {
		if (count > maxGroupItems()) {
			throw new Error(`conveyor: ${kind} item cap exceeded (${count} > MAX_GROUP_ITEMS=${maxGroupItems()})`);
		}
	}
	...
	async #effect(name, input, opts = {}) {
		...
		if (!host || !fn) throw new Error(`conveyor: no host effect '${name}'`);
	...
	#worktreeApi() {
		...
		const worktree = this.restricted
			? () => {
					throw new Error("conveyor: worktree() is forbidden in restricted mode");
			  }
...
export function applyRunSettings(o, defaults = {}) {
	...
	if (error) throw new Error(`conveyor: ${error}`);
	return spec;
}
...
function applyAgentProfile(o, inherited) {
	...
	if (!["inherit", "none", "read-only", "research"].includes(profile)) {
		throw new Error(`conveyor: unknown agent profile '${profile}'`);
	}
	...
	if (paths.length > 1) throw new Error("conveyor: one permissions.paths root is supported per agent; use separate agents for disjoint roots");
...
function stringListOption(value, name) {
	...
	if (!values.every((item) => typeof item === "string" && item.length > 0)) {
		throw new Error(`conveyor: ${name} must be a string or array of non-empty strings`);
	}
	return values;
}
