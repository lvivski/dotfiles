 ...
 * @module sdk
 *
 * SDK-backed conveyor agents. One run backend lazily owns one Copilot client per cwd/MCP
...
/** Create one lazy persistent SDK backend owned by a conveyor run. */
...
export function createSdkRunBackend(sdk, defaults = {}) {
	...
	const backend = {
		...
		async run(/** @type {AgentSpec} */ input, /** @type {{ signal?: AbortSignal }} */ opts = {}) {
			const spec = input;
			if (!accepting) return agentResult(spec, failedStart(spec, "conveyor SDK backend is closed"));
			/** @type {(() => void)|null} */
			let abort = null;
...
async function configureSession(session, spec, policy, cancellable) {
	...
	// SDK allow-all covers tools, paths, and URLs together. Keep it off and express the conveyor's
	...
	if (rules.approved.length || rules.denied.length) {
		...
		if (configured?.success === false) throw new Error("child session refused conveyor profile permission rules");
	}
}
...
function createPermissionPolicy(spec) {
	...
	return {
		...
		decide(/** @type {any} */ request) {
			...
			if (deniedRule) return { kind: "reject", feedback: `Permission '${formatPermissionRule(deniedRule)}' is denied by the conveyor agent profile.` };
			if (request?.kind === "url" && request.requestSandboxBypass) {
				return { kind: "reject", feedback: "Conveyor agents cannot auto-approve sandbox bypass requests." };
			}
			...
			return allowed
				? { kind: "approve-once" }
				: { kind: "reject", feedback: "Permission is not approved by the conveyor agent profile." };
		},
	};
}
