...
test("keepSessions follows CONVEYOR_KEEP_SESSIONS", async () => {
	await withFakeEnv({}, () => assert.equal(keepSessions(), false));
	await withFakeEnv({ CONVEYOR_KEEP_SESSIONS: "1" }, () => assert.equal(keepSessions(), true));
});
...
test("CONVEYOR_KEEP_SESSIONS=1 preserves everything", async () => {
	await withFakeEnv({ CONVEYOR_KEEP_SESSIONS: "1" }, async () => {
		makeSessionDir("child-1");
		const rt = stubRuntime([mkResult({ sessionId: "child-1" })]);
		await rt.agent("hi");
		const res = await rt.cleanupSessions({ status: "complete" });
