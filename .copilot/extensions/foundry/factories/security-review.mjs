...
export const meta = {
	...
	description:
		"Investigate a repository scope from independent security perspectives and verify findings. " +
		"Args: { root?: string, files?: string[], base?: string, head?: string, perspectives?: number, " +
		"context?: string, model?: string }.",
	phases: [
		{ title: "Orient" },
		{ title: "Investigate" },
		{ title: "Verify" },
...
factory.phase("Orient");
const threatModel = await factory.agent(
...
factory.phase("Investigate");
const investigations = await factory.parallel(
...
const verified = await factory.pipeline(candidates, async (finding, _original, index) => {
