/**
 * Browser renderer for the Foundry loopback canvas.
 *
 * @module foundry/renderer
 */
 ...
 * @property {any} plan Authoritative Foundry plan document.
...
/**
 * Calls a loopback API endpoint and unwraps the Foundry response envelope.
 *
 * @param {string} path
 * @param {RequestInit} [options]
 * @returns {Promise<any>}
 * @throws {Error} When the server returns a failed Foundry envelope.
 */
...
function renderRecovery() {
    ...
    if (currentPlan.cancellation) {
        ...
        host.append(element(
            "p",
            "warning",
			"Foundry has only requested cancellation. Stop or archive every listed App session and cancel the authoritative Factory run before finalization.",
        ));
    }
