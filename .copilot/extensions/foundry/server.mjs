/**
 * Hardened loopback HTTP and SSE server for one Foundry canvas instance.
 *
...
/**
 * @typedef {object} FoundryServerEntry
 * @property {string} instanceId
 * @property {string} planId
...
class FoundryHttpError extends Error {
    /**
...
async function readJson(request) {
    ...
    if (contentType !== "application/json") {
        throw new FoundryHttpError(415, "Content-Type must be application/json");
    }
    let size = 0;
    const chunks = [];
    for await (const chunk of request) {
        size += chunk.length;
        if (size > BODY_LIMIT) {
            throw new FoundryHttpError(413, "Request body exceeds the 32 KiB limit");
        }
        chunks.push(chunk);
    }
    try {
        ...
        throw new FoundryHttpError(400, "Request body must contain valid JSON");
    }
}
...
/**
 * Maps Foundry failures to HTTP response status codes.
 *
 * @param {any} error
 * @returns {number}
 */
...
function publicError(error) {
    return {
        code: error?.code ?? "foundry_canvas_error",
 ...
 *
 * @param {FoundryServerOptions} options
 * @returns {Promise<FoundryServerEntry>}
 */
export async function startServer(options) {
    ...
    const server = createServer(async (request, response) => {
        ...
        try {
            if (request.method === "GET" && (url.pathname === "/" || url.pathname === "/index.html")) {
                ...
                const html = template
                    .replaceAll("__FOUNDRY_TOKEN__", actionToken)
                    .replaceAll("__FOUNDRY_PLAN_ID__", planId);
            ...
            if (request.method === "POST" && url.pathname === "/api/action") {
                if (!tokenMatches(actionToken, request.headers["x-foundry-token"])) {
    ...
    if (!address || typeof address === "string") {
        server.close();
        throw new Error("Foundry canvas did not receive a loopback port");
    }
