/** @module work — one durable lifecycle concept for a real workflow attempt. */
...
export class Work {
	...
	/**
	 * Open one real workflow attempt. Dry runs must never call this.
	 * @param {WorkOptions} input
	 */
	static open(input) {
		if (ACTIVE.has(input.runId)) throw new WorkError(`workflow run '${input.runId}' is already active.`);
...
export function requestWorkControl(runDir, action) {
	if (!ACTIONS.has(action)) throw new WorkError(`unsupported workflow control action '${action}'`);
	const owner = readWorkOwner(runDir);
	if (!owner || !processIsAlive(owner.pid)) throw new WorkError("workflow has no live owner");
	const id = randomUUID();
	const request = {
		id,
		action,
	...
	try {
		...
		throw new WorkError(`failed to persist workflow control request: ${error instanceof Error ? error.message : error}`);
	}
	const current = readWorkOwner(runDir);
	if (!sameOwner(owner, current) || !processIsAlive(owner.pid)) {
		removeArtifact(path);
		throw new WorkError("workflow ownership changed before the control request was accepted");
	}
	return request;
}
...
export function reconcileWorkRecord(rec, runDir) {
	...
	const owner = readWorkOwner(runDir);
	if (owner) {
		if (!processIsAlive(owner.pid)) return interrupted(rec, "workflow host process exited before completion");
		if (owner.pid === process.pid && owner.instanceId && owner.instanceId !== PROCESS_INSTANCE_ID) {
			return interrupted(rec, "workflow owner instance exited before completion");
		}
	...
	if (Number.isInteger(pid) && pid > 0) {
		if (!processIsAlive(pid)) return interrupted(rec, "workflow host process exited before completion");
		if (pid === process.pid && rec.ownerInstanceId !== PROCESS_INSTANCE_ID) return interrupted(rec, "workflow owner instance exited before completion");
	}
	const updated = timestampMs(rec.heartbeatAt || rec.updatedAt || rec.startedAt);
	if (updated > 0 && Date.now() - updated > HEARTBEAT_STALE_MS) return interrupted(rec, "workflow has no active owner lock");
	return rec;
}
