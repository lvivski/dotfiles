...
const target = typeof context.args === "string" ? context.args : "the codebase";

const done = context.memory.read(); // everything prior runs recorded ("" on the first run)
if (!context.memory.enabled) log("loop-memory: no memory file set — this run's note will not persist.");
...
if (note) {
	context.memory.append("- " + note.split("\n")[0]); // persist for the next run
	return note;
}
